...
export default function CoursePlayer({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function complete() {
    if (!lesson) return;
    const r = await api('/members/progress', { method: 'POST', body: JSON.stringify({ lessonId: lesson.id, completed: true, positionSec: lesson.durationSec }) });
  ...
  return (
    <main className="grid min-h-screen md:grid-cols-[280px_1fr]">
      <aside className="border-r border-ink/10 p-4">
        {data.course.modules.map((m: any) => (
          <div key={m.id} className="mb-4">
            <p className="text-xs uppercase tracking-widest text-ink/40">{m.title}</p>
            {m.lessons.map((l: any) => (
              <button key={l.id} onClick={() => setLesson(l)} className={`mt-1 block w-full rounded-lg px-2 py-1 text-left text-sm ${lesson?.id === l.id ? 'bg-gold' : ''}`}>{l.title}</button>
            ))}
          </div>
        ))}
      </aside>
      <section className="p-8">
        ...
        <button className="btn-primary mt-6" onClick={complete}>Marcar como concluída</button>
      </section>
    </main>
  );
}
