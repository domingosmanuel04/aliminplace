...
export class AuthService {
  ...
  async login(dto: LoginDto, req: any, res: any) {
    ...
    if (user.twoFactorEnabled) {
      if (
        ...
        !authenticator.check(dto.otp, user.twoFactorSecret)
      ) {
  ...
  async me(userId: string) {
    const user = await this.prisma.user.findUnique({
      where: { id: userId },
      include: {
        memberships: { include: { tenant: { include: { stores: true } } } },
      },
    });
    if (!user) throw new UnauthorizedException();
  ...
  async confirm2fa(userId: string, otp: string) {
    ...
    if (
      !user?.twoFactorSecret ||
      !authenticator.check(otp, user.twoFactorSecret)
    ) {
  ...
  async createStoreOnboarding(
    ...
    const tenant = await this.prisma.tenant.create({
      data: {
        ...
        warehouses: {
          create: { name: "Principal", isDefault: true, city: "Luanda" },
        },
      },
      include: { stores: true },
    });
  ...
  private async issue(user: any, req: any, res: any) {
    ...
    const access = await this.jwt.signAsync(
      ...
      { secret: accessSecret, expiresIn: process.env.JWT_ACCESS_TTL || "15m" },
    );
