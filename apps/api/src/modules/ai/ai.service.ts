...
export class AiService {
  ...
  async copilot(prompt: string) {
    ...
    if (lower.includes("checkout"))
      return {
        type: "checkout",
        ...(await this.optimizeCheckout()),
        insights: insights.notes,
      };
    if (lower.includes("loja") || lower.includes("store"))
      return { type: "store", ...(await this.buildStore(prompt)) };
    if (lower.includes("curso") || lower.includes("produto")) {
      const product = await this.buildProduct(prompt);
      return { kind: "product", ...product, insights: insights.notes };
    }
    return {
      type: "insights",
  ...
  async buildStore(brief: string) {
    ...
    const palette =
      brief.toLowerCase().includes("premium") ||
      brief.toLowerCase().includes("luxo")
  ...
  async buildProduct(brief: string) {
    ...
    const name = isCourse
      ? `Curso: ${brief.replace(/quero vender/i, "").trim()}`
      : brief.slice(0, 80);
    const modules = isCourse
      ? [
          {
            title: "Fundamentos",
            lessons: ["Boas-vindas", "Ferramentas", "Primeiros passos"],
          },
          {
            title: "Prática",
            lessons: ["Exercício 1", "Exercício 2", "Projecto"],
          },
          {
            title: "Avançado",
            lessons: ["Atalhos", "Erros comuns", "Certificação"],
          },
        ]
      : [];
    return {
      ...
      faq: [
        ...
        {
          q: "Há certificado?",
          a: isCourse ? "Sim, após concluir 100% e o quiz." : "Não aplicável.",
        },
      ],
  ...
  async optimizeCheckout() {
    ...
    return {
      ...
      recommendations: [
        {
          area: "prova social",
          action: "Adicionar 3 depoimentos com foto acima do pagamento",
        },
        {
          area: "fricção",
          action: "Pedir apenas nome, email e telefone no primeiro passo",
        },
        {
          area: "urgencia",
          action: "Countdown real ligado a stock ou campanha",
        },
        {
          area: "order bump",
          action: "Oferecer um digital de baixo ticket no checkout",
        },
      ],
    };
  }
  ...
  private guessCategories(brief: string) {
    ...
    if (b.includes("roupa") || b.includes("moda"))
      return ["Novidades", "Homem", "Mulher", "Acessórios"];
    if (b.includes("fit") || b.includes("treino"))
      return ["Cursos", "Suplementos", "Roupa"];
    if (b.includes("comida") || b.includes("culin"))
      return ["Cursos", "Alimentos", "Utensílios"];
