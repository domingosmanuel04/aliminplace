...
export class LearningService {
  ...
  async myEnrollments(userId: string) {
    return this.prisma.enrollment.findMany({
      where: { userId },
      include: {
        course: { include: { product: { include: { media: true } }, modules: { include: { lessons: true } } } },
        certificates: true,
      },
    });
  }
  ...
  async courseForStudent(courseId: string, userId: string) {
    const enrollment = await this.prisma.enrollment.findUnique({
      ...
      include: {
        course: { include: { modules: { include: { lessons: { orderBy: { position: 'asc' } } }, orderBy: { position: 'asc' } }, quizzes: { include: { questions: true } }, product: true } },
        progressItems: true,
      },
    });
  ...
  async saveProgress(userId: string, lessonId: string, positionSec: number, completed: boolean) {
    ...
    const total = await this.prisma.lesson.count({ where: { module: { courseId: enrollment.courseId } } });
    const done = await this.prisma.lessonProgress.count({ where: { enrollmentId: enrollment.id, completed: true } });
    const progress = total ? (done / total) * 100 : 0;
    await this.prisma.enrollment.update({
      where: { id: enrollment.id },
      data: { progress, lastLesson: lessonId, completedAt: progress >= 100 ? new Date() : null },
    });
    if (progress >= 100) {
  ...
  async attemptQuiz(userId: string, quizId: string, answers: Record<string, string>) {
    ...
    if (attempts >= quiz.maxAttempts) throw new ForbiddenException('Tentativas esgotadas');
    let score = 0;
    let total = 0;
    for (const q of quiz.questions) {
      total += q.points;
      if ((answers[q.id] || '').trim().toLowerCase() === q.answer.trim().toLowerCase()) score += q.points;
    }
    const pct = total ? Math.round((score / total) * 100) : 0;
    return this.prisma.quizAttempt.create({
      data: { quizId, userId, score: pct, passed: pct >= quiz.passScore, answers },
    });
  }
  ...
  async saveCurriculum(productId: string, modules: any[]) {
    ...
    if (!course) course = await this.prisma.course.create({ data: { productId } });
    await this.prisma.courseModule.deleteMany({ where: { courseId: course.id } });
    for (const [i, m] of modules.entries()) {
      const mod = await this.prisma.courseModule.create({
        data: { courseId: course.id, title: m.title, position: i + 1, locked: !!m.locked },
      });
    ...
    return this.prisma.course.findUnique({ where: { id: course.id }, include: { modules: { include: { lessons: true } } } });
  }
}
