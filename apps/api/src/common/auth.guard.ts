...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      ...
      if (!user || user.status !== 'ACTIVE') throw new UnauthorizedException('Conta inactiva');

      const tenantId = (req.headers['x-tenant-id'] as string) || payload.tenantId;
      ...
      if (tenantId) {
        ...
        if (member) {
          permissions = member.permissions.includes('*') ? ['*'] : member.permissions;
          role = member.role;
        } else if (!user.isSuperAdmin) {
          if (!isPublic) throw new ForbiddenException('Sem acesso a este tenant');
        }
      }
      ...
      if (this.reflector.getAllAndOverride<boolean>(SUPER, [context.getHandler(), context.getClass()])) {
        if (!user.isSuperAdmin) throw new ForbiddenException('Apenas super admin');
      }

      const needed = this.reflector.getAllAndOverride<string[]>(PERMS, [context.getHandler(), context.getClass()]);
